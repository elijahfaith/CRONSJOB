...
# Function to check if hub-tool is installed
check_hub_tool() {
    echo "Checking if hub-tool is installed..."
    if ! command -v hub-tool &> /dev/null; then
        echo "hub-tool is not installed. Installing..."
        
        # Install hub-tool
        curl -L https://github.com/rofrischmann/hub-tool/releases/latest/download/hub-tool-linux -o hub-tool
        chmod +x hub-tool
        sudo mv hub-tool /usr/local/bin/

        if command -v hub-tool &> /dev/null; then
            echo "hub-tool successfully installed."
        else
            echo "Failed to install hub-tool. Please install it manually and re-run the script."
            exit 1
        fi
    else
        echo "hub-tool is already installed."
    fi
}

# Function to log in to Docker Hub using hub-tool CLI
hub_tool_login() {
    echo "Logging in to Docker Hub using hub-tool..."

    # Log in with hub-tool CLI
    hub-tool login "$DOCKERHUB_USERNAME"
...
monitor_logs() {
    ...
    # Fetch list of repositories for the user
    REPOSITORIES=$(hub-tool repos list --format '{{.Name}}')

    # Check if fetching repositories was successful
    if [ $? -ne 0 ]; then
        echo "Error fetching repositories. Please ensure hub-tool is configured properly."
        exit 1
    fi
    ...
    while IFS= read -r REPO; do
        ...
        # Get details for the specific repository and format with jq
        LOG_DETAILS=$(hub-tool repos get "$DOCKERHUB_USERNAME/$REPO" | jq -r '.')
...
# Main script execution
check_hub_tool    # Check if hub-tool is installed and install it if necessary
hub_tool_login    # Log in to Docker Hub using hub-tool
monitor_logs      # Run the monitoring process and save output to CSV
